		...
		if len(asteroids) == 0:
			...
			if playership['shield'] > 0:
				...
				for ast in range(0,gamelevel):
					...
					if maxspeed > 3:
						maxspeed = 3
					asteroids.append({'x':float(random.randint(-100,100)), 'y':float(random.randint(-100,100)), 'dx':float(random.randint(-maxspeed,maxspeed))+0.5, 'dy':float(random.randint(-maxspeed,maxspeed))+0.5, 'frame':random.randint(0,10), 'scale':1.0, 'angle':random.randint(0,359)})
